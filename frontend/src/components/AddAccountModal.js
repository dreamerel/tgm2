...
function AddAccountModal({ open, onClose }) {
  ...
  const handleSendCode = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post('https://tgm2.onrender.com/api/telegram/send_code', { phone, api_id: apiId, api_hash: apiHash });
  ...
  const handleVerifyCode = async (e) => {
    ...
    try {
      const res = await axios.post(
        'https://tgm2.onrender.com/api/telegram/sign_in',
  ...
  const handleFinish = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post('https://tgm2.onrender.com/api/telegram/sign_in', { phone, code, password, api_id: apiId, api_hash: apiHash });
      if (res.data && res.data.ok) {
        onClose();
        reset();
      } else {
